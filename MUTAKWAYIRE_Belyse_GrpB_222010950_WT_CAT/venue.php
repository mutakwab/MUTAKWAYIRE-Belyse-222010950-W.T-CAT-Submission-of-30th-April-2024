...
<section>
  <center>
  <h1><u> Venue Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
       <!--venue_id name  address capacity-->     
        <label for="venid">venue id:</label>
        <input type="number" id="venid" name="venid"><br><br>

        <label for="vename">name:</label>
        <input type="text" id="vename" name="vename" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO venue(venue_id, name, address, capacity) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $VenId, $VenName, $VenAdrs, $VenCapacity);
    // Set parameters and execute
    $VenId = $_POST['venid'];
    $VenName = $_POST['vename'];
    $VenAdrs = $_POST['address'];
    $VenCapacity = $_POST['capacity'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of venue</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of venue</h2></center>
    <table border="5">
        <tr>
            <th>venue id</th>
            <th>name</th>
      ...
      include('db_connection.php');
        ...
        // Prepare SQL query to retrieve all venue
        $sql = "SELECT * FROM venue";
        $result = $connection->query($sql);

        // Check if there are any venue
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $venid = $row['venue_id']; // Fetch the venue_Id
                echo "<tr>
                    <td>" . $row['venue_id'] . "</td>
                    <td>" . $row['name'] . "</td>
                    <td>" . $row['address'] . "</td>
                    <td>" . $row['capacity'] . "</td>
                    <td><a style='padding:4px' href='delete_venue.php?venue_id=$venid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_venue.php?venue_id=$venid'>Update</a></td> 
                </tr>";
            }
        } else {
